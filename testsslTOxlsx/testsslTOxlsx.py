            ...
            for values in entry["scanResult"]:
                for protocol in values["protocols"]:
                    if protocol["id"] in protocols:
                        if protocol["finding"].split(' ', 1)[0] == "offered" or protocol["finding"].split(' ', 1)[0] == "vulnerable":
                            table_data.append(
                                [
                                    values["ip"],
                                    int(values["port"]),
        ...
        for entry in data:
            for values in entry["scanResult"]:
                ...
                for protocol in values["protocols"]:
                    if protocol["id"] in protocols:
                        if protocol["finding"].split(' ', 1)[0] == "offered" or protocol["finding"].split(' ', 1)[0] == "vulnerable":
                            d[protocol["id"]] = protocol["severity"]
                        else:
                            d[protocol["id"]] = "NO"
...
def parse_host_ciphers(workbook, data):
    ...
    for cipher in ciphers:
        table_headers.append({"header": cipher})
        ...
        for entry in data:
            for values in entry["scanResult"]:
                ...
                for cipher in values["ciphers"]:
                    if cipher["id"] in ciphers:
                        if cipher["severity"] != "OK":
                            d[cipher["id"]] = cipher["severity"]
                        else:
                            d[cipher["id"]] = "NO"
...
def main():
    ...
    logging.info("generating worksheet 'Host vs Ciphers'...")
